...
class Adm extends CI_Controller
{
    ...
    public function h_ujian()
    {
        ...
        if ($uri3 == "det") {
            ...
            $a['statistik'] = $this->db->query("SELECT *, MAX(nilai) AS max_, MIN(nilai) AS min_, AVG(nilai) AS avg_ 
											FROM tr_ikut_ujian
											WHERE tr_ikut_ujian.id_tes = '$uri4' GROUP BY id")->row();
            //get all user's answe
            $a['detail_jwb'] = $this->db->query("SELECT * FROM tr_ikut_ujian WHERE id_tes = '$uri4'")->result();
            ...
            foreach ($q_datanya as $d) {
                ...
                $data_ok[5] = '<a href="' . base_url() . 'adm/h_ujian/batalkan_ujian/' . $d['id'] . '/' . $this->uri->segment(4) . '" class="btn btn-danger btn-xs" onclick="return confirm(\'Anda yakin...?\');"><i class="glyphicon glyphicon-remove" style="margin-left: 0px; color: #fff"></i> &nbsp;&nbsp;Batalkan Ujian</a> || <a class="btn btn-primary btn-xs" target="_blank" href="' . base_url() . 'adm/detailjawaban/' . $d['id'] . '">Detail Jawaban</a>';
    ...
    */
    ...
    public function detailJawaban()
    {
        ...
        //query
        $data['info'] = $this->db->query("SELECT *, m_mapel.nama as nama_mapel, m_siswa.nama as nama_siswa FROM `tr_ikut_ujian` JOIN tr_guru_tes ON tr_ikut_ujian.id_tes = tr_guru_tes.id JOIN m_siswa ON tr_ikut_ujian.id_user = m_siswa.id JOIN m_mapel ON m_mapel.id = tr_guru_tes.id_mapel WHERE tr_ikut_ujian.id = $id")->row_array();

        //pecahkan no soal
        $id_soal = explode(',', $data['info']['list_soal']);
        //print_r($id_soal);
        $dataSoal = [];
        for ($x = 0; $x < count($id_soal); $x++) {
            //echo $x;
            $dataSoal[] = $this->db->query("SELECT * FROM m_soal WHERE id = $id_soal[$x]")->row_array();
        }
        ...
        //soal :: jawaban
        $hasil = explode(',', $data['info']['list_jawaban']);
        $data['jawaban'] = $hasil;
        /*echo "<pre>";
        print_r($data);
        echo "</pre>";*/

        $this->load->view('detailJawabanUjian', $data);
